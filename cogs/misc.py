...
class MiscCog(commands.Cog):
    ...
    @tuna_admin.command(name="massrole")
async def tuna_massrole(ctx: commands.Context, source_role: discord.Role, target_role: discord.Role):
    """Give every member with source_role the target_role.
    Usage: !tuna_admin massrole @SourceRole @TargetRole"""
    ...
    if dangerous:
        await ctx.send(
            ...
            f"Cannot assign roles with elevated permissions."
        )
        return

    if target_role >= guild.me.top_role:
        await ctx.send(f"❌ **{target_role.name}** is at or above my highest role. I can't assign it.")
        return
    ...
    token = __import__("uuid").uuid4().hex[:8].upper()
    await ctx.send(
        f"About to give **{target_role.mention}** to **{len(members)}** member(s) with **{source_role.mention}**.\n"
        f"Confirm by typing the token in this channel:\n```{token}```",
        allowed_mentions=discord.AllowedMentions.none()
    )
    ...
    for member in members:
        try:
            await member.add_roles(target_role, reason=f"tuna_admin massrole by {ctx.author}")
            success += 1
        except Exception:
            failed += 1
    ...
    if failed:
        lines.append(f"⚠️ Failed for {failed} member(s).")
